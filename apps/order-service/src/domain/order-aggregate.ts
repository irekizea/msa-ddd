...
export class Order {
    ...
    addItem(productId: string, quantity: number, unitPrice: number) {
        if (this._status !== 'CREATED') throw new Error('Cannot modify items after payment/cancel');
        if (quantity <= 0) throw new Error('quantity > 0');
        if (unitPrice < 0) throw new Error('unitPrice >= 0');
        this._items.push({ productId, quantity, unitPrice, lineTotal: quantity * unitPrice });
    }
    ...
    // getters
    id() { return this._id; }
    customerId() { return this._customerId; }
    currency() { return this._currency; }
    items() { return this._items.slice(); }
    totalAmount() { return this._totalAmount; }
    status() { return this._status; }
    version() { return this._version; }

    // rehydrate (infra can use this)
    static fromPrimitives(p: {
        id: string; customerId: string; currency: Currency; status: OrderStatus; items: OrderItem[]; totalAmount: number; version: number;
    }) {
    ...
    toPrimitives() {
        return { id: this._id, customerId: this._customerId, currency: this._currency, status: this._status, items: this._items, totalAmount: this._totalAmount, version: this._version };
    }
}
